...
For one unit's per-cycle table this computes, on the data:
  ...
  * the three candidate families fitted over positions 1..min(T, N), the per-unit best by in-sample RMSE,
...
def audit_unit(df: pd.DataFrame, *, column: str, rule: CleaningRule, q_nom: float, decl: dict, rho_grid, k_grid,
    ...
    pos = d["position"].to_numpy(float)[:end]
    y = q[:end] / st.q1
    ...
    rec["_trace"] = {"position": pos, "q_raw": q[:end], "q_smooth": qs, "fit": FAMILIES[best](fits[best].params, pos) * st.q1,
                     "residual": resid}
    return rec
