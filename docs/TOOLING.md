...
| Permutation and conditional permutation importance, term ablation | `degradx.metrics.usability` + `sklearn.linear_model.RidgeCV`, `scipy.stats.bootstrap` | the null-channel gate and the conditional importance of redundant channels (C2) are benchmark checks on window regressors with a unit-level interval; `sklearn.inspection.permutation_importance` permutes single tabular columns and has no conditional variant or unit-level resampling |
| Degradation operators on attribution maps | `degradx.metrics.scores.OPERATORS` + `scipy.ndimage.gaussian_filter1d` | the operators of §3.5.3 (noise, shifted mass, smoothing, permutation) are defined by the protocol; no package degrades a known-correct map |
| Resolution and operating-range rules | `scripts/s7_responsiveness.py` (`resolution`, `operating_range`) + `scipy.stats.bootstrap` | declared benchmark rules (declarations `responsiveness`) |
| Attribution wrappers | `degradx.attribution.methods` (captum `IntegratedGradients`, `FeatureAblation`; timeshap `TimeShapKernel`) | adapters to the raw-channel window input and the declared pristine baseline; the attribution itself is the packages' |
| Reference model | `degradx.attribution.methods.ReferenceModel` (torch) | Eq. 5's affine function with the ground-truth weights; one line of torch so the same attribution code runs on it |
